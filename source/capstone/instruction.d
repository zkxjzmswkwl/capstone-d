...
public:
    ...
    auto bytes() const
    {
        return internal.bytes[0 .. internal.size];
    }
    ...
    auto mnemonic() const
    {
        return internal.mnemonic.ptr.to!string;
    }
    /// Ascii text of instruction operands
    auto opStr() const
    {
        return internal.op_str.ptr.to!string;
    }
...
*/
abstract class InstructionImpl(TId, TRegister, TDetail) : Instruction
        if (is(TId == enum))
{ // TODO: isRegister, isDetail
    ...
    auto id() const
    {
        return internal.id.to!TId;
    }

    override const(TDetail) detail() const
    {
        if (_detail.isNull)
            throw new CapstoneException("Trying to access unavailable instruction detail", ErrorCode
    ...
    private const(TRegister)[] accessedRegs(in bool writeAccess) const
    {
        ...
        if (diet)
            throw new CapstoneException(
                "Registers accessed by an instruction are not stored when running Capstone in diet mode",
                ErrorCode.IrrelevantDataAccessInDietEngine);
        cs_regs read, write;
    ...
    override const(TRegister)[] writes() const
    {
        return accessedRegs(true);
    }

    override const(TRegister)[] reads() const
    {
        return accessedRegs(false);
    }
}
